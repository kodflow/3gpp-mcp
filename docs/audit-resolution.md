...
| **F15** | `mean_pool` windowing (#208) is still not ported to Rust | `CONFIRMED` | The canonical windowing is `truncate@1024`, consistently, in Rust, in Go and in `contracts/identity.toml`; a mismatch is impossible because the Go `mean_pool` is unreachable. Flipping it bumps the EmbedIdentity and costs a full re-embed, so it is a decision to take deliberately, before a campaign — not during one. |
