...
# --- build embed-core --features ort,cuda + the check bin --------------------
b = sh(f"cd {src} && cargo build --release --manifest-path rust/embed-core/Cargo.toml --features ort,cuda --bin embed-core-check")
if b.returncode != 0:
    fail("cargo build embed-core --features ort,cuda")

# --- run the check on GPU with the real model --------------------------------
env = os.environ.copy()
...
with open(os.path.join(WORK, "ffi-check.json"), "w") as f:
    json.dump({"ok": ok, "branch": BRANCH, "queries": QUERIES, "results": results}, f, indent=2)
print("FFI-CHECK " + ("OK" if ok else "FAIL") + f" ({len(results)}/{len(QUERIES)} queries embedded via the real BGE-M3 ort path on GPU)", flush=True)
