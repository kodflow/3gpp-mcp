...
stage_kernel() {
  ...
  {
    # The lexical base is a PRIVATE GHCR image now, so the kernel needs a
    # read:packages token: it ALWAYS fresh-slices from the base, then carries prior
    # vectors over (from the mounted resume Dataset, or — when that Dataset was
    # deleted — from the durable GHCR 3gpp-vec channel). The migration made this
    # credential unavoidable (decision: all-GHCR, no public Release) — the kernel
    # stays is_private:true and the PAT is fed to crane on stdin, never echoed.
  ...
  # when no RESUME_DB is present (and, if the series was already published, carries
  # its vectors over from GHCR 3gpp-vec — a deleted Dataset never costs a full
  # re-embed). `recover_orphan` repopulates state-$s from Kaggle before each push,
  # so this local file is a faithful proxy for "resume published".
  local sources=""
